...
void loadconfig(Config *conf)
{
    ...
    while (fgets(buf, MAX_CONF_LEN+1, fp) != NULL) {
        ...
        if (argc == 2) {
            if (strcasecmp(argv[0], "url") == 0) {
                conf->url = strdup(argv[1]);
            } else if (strcasecmp(argv[0], "vurl") == 0) {
                conf->vurl.push_back(strdup(argv[1]));
            } else if (strcasecmp(argv[0], "module_path") == 0) {
                conf->module_path = strdup(argv[1]);
            } else if (strcasecmp(argv[0], "xml_modules") == 0) {
                conf->xml_modules.push_back(strdup(argv[1]));
            } else if (strcasecmp(argv[0], "load_modules") == 0) {
                conf->load_modules.push_back(strdup(argv[1]));
            } else {
