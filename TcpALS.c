...
int main(int argc, char *argv[])
{
    ...
    /*初始化进程*/
    if (tpTaskInit(atoi(argv[1]), "TcpALS", &m_tpTask) != 0) {
        printf("初始化进程出错\n");
        return -1;
    }
    ...
    for (;;){
        ...
        for (;;){
            ...
            */
            /* del by dwxiong @2015-3-20 10:43:55 */
            ...
            if ((ret = mq_post(mqh, NULL, q_des, 0, NULL, (char *)&tpMsg, msgLen, 0)) != 0) {
                LOG_ERR("mq_post() error: retcode=%d", ret);
                LOG_ERR("写主控队列[%d]出错", q_des);
                continue;
            }
