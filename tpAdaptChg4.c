...
int  m_fmtType;                /*是否初始化报文头*/ /*add by dwxiong 2016-11-07 15:11:24*/
...
int tpParamInit()
{
    ...
    if (tpGetParamInt(m_tpTask.params, "fmtType", &m_fmtType) != 0) m_fmtType = 1; /*default MSG_NVL=1*/
    LOG_MSG("参数[fmtType]=[%d]", m_fmtType);
...
void *tpProcessThread(void *arg)
{
    ...
    while (1) {
        ...
        if (m_src_q == q || Q_REV == q ) {
            ...
            if ((ret = mq_post(mqh, NULL, q_tmp, 0, NULL, (char *)&tpMsg, msgLen, 0)) != 0) {
                LOG_ERR("mq_post() error: retcode=%d", ret);
                LOG_ERR("写队列[%d]出错",q_tmp);
            }
            LOG_MSG("转发报文[%d]==>[%d]...[ok]", q,q_tmp);
            continue;        
        }
        else if (m_des_q == q) {
