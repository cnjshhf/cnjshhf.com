...
int tpParamInit()
{
    ...
    if (tpGetParamInt(m_tpTask.params, "retocde", m_retcode) != 0) memcpy(m_retcode,"00",2);
    LOG_MSG("参数[retocde]=[%s]", m_retcode);
...
void *tpProcessThread(void *arg)
{
    ...
    while (1) {
        ...
        if (m_src_q == q || Q_SVR == q) {
            memcpy(tpMsg.body+178,m_retcode,2);
