...
void *tpRequestThread(void *arg)
{
    ...
    char buff[MAXMSGLEN-sizeof(TPHEAD)];
    long blen;
    ...
    while (1) {
        ...
        /*接收请求报文*/
        /* 增加base64解码 */
        if ((blen = tcp_recv_h(pSock->sockfd, buff, 0, m_lenlen, 0)) <= 0) {
            if (!sk_f)
        ...
        msgLen = Base64Decode(tpMsg.body, buff, blen);
        if(msgLen == -1)
        {
           	LOG_ERR("接收客户端请求报文长度出错[%ld]",blen);
            continue;   
        }
...
void *tpResponseThread(void *arg)
{
    ...
    while (1) {
        ...
        if (tcp_send(pSock->sockfd, b64msg, b64len, m_lenlen, 0) == -1) {
